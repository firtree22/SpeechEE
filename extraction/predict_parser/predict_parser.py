...
class Metric:
    ...
    def countTI(self, gold_list, pred_list, verbose=False):  #trigger 转成小写后一致
        # print("CountTI")
        if verbose:
            print("Gold:", gold_list)
            print("Pred:", pred_list)
    ...
    def countTC(self, gold_list, pred_list, verbose=False): #只要保证Event Type正确即可    'gold_event': [('Nominate', 'named')], 'pred_event': [('Start-Position', 'named')],
    ...
    def countAC(self, gold_list, pred_list, verbose=False):  #Event type正确 Role Type正确
        if verbose:
            print("Gold:", gold_list)
            print("Pred:", pred_list)
        self.gold_num += len(gold_list)
